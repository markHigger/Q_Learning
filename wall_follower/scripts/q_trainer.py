...
EPSILON = 0.98 #Random Learning for eps greedy
...
def saveR(R_vec):
    filename = './R_vec_q'
    with open(filename, 'wb') as handle:
        pickle.dump(R_vec, handle, protocol=pickle.HIGHEST_PROTOCOL)
def controlBot():
    ...
    while not rospy.is_shutdown():
        ...
        while(~np.isnan(currentRegions[0])):
            # rospy.loginfo(currentRegions)
            state = getState(currentRegions)
            #Terminate when bot is too close
            if state == '-1' or ittIdx > 2000:
            ...
            if not Q.has_key(state):
                Q[state] = [0, 0, 0, 0, 0]
            if not R.has_key(state):
                R[state] = -1

            # rospy.loginfo(state)
            # rospy.loginfo(Q[state])
            eps = EPSILON * pow(0.97, epIdx)
            ...
            if not R.has_key(nextstate):
                R[nextstate] = -1

            # rospy.loginfo([ETA * (np.max(Q[nextstate])), R[nextstate]])
            # Q[state][action] = R[nextstate] + (ETA * (np.max(Q[nextstate])))
            Q[state][action] = Q[state][action] + ETA * (R[nextstate] + (GAMMA * (np.max(Q[nextstate]))) - Q[state][action])
            R_cum += R[nextstate]
            saveQ(Q)
            # rospy.loginfo(Q)
            # printQ(Q)
